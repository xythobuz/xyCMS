...
if (basename($_SERVER['PHP_SELF']) == "render.php") {
    include("config.php");
    header('Location: '.$xyCMS_root.'/stats.php');
    exit;
}
...
if (!$result) {
    ...
    imagestring($img, 2, (WIDTH - 145), (HEIGHT - 20), "rendered by xyCMS", $grey);
    imagepng($img);
    exit;
}
...
if ($renderVisitors) {
    ...
    imagestring($img, 4, 3, 3, "Bots ".date('m.Y')." ".$xyCMS_title, $black);
} else {
    imagestring($img, 4, 3, 3, "Views ".date('m.Y')." ".$xyCMS_title, $black);
}
...
imagestring($img, 2, 4, 17, "rendered by xyCMS", $grey);
imagepng($img);
